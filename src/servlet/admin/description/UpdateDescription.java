...
public class UpdateDescription extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if(image == null){
			description.setImage((String) request.getAttribute("image"));
		}else{
		...
		if(introduce == null){
			description.setIntroduce((String)request.getAttribute("introduce"));
		}else{
			description.setIntroduce(introduce);
			String path = (String) request.getAttribute("introduce");
			String path1 = getServletContext().getRealPath(path);
			File file = new File(path1);
			if (file.exists()) {
				file.delete();
		...
		description.setId(Integer.parseInt((String)request.getAttribute("id")));
		description.setName((String)request.getAttribute("name"));
		description.setInfo((String)request.getAttribute("info"));
		
		int result = Description.update(description);
		if(result > 0){
			response.sendRedirect("FindAllDescription?page=1");
		}else{
			out.print("<script>alert('修改 "+description.getName()+" 失败，请重新修改！');history.go(-1);</script>");
		}
		out.flush();
		out.close();
	}
